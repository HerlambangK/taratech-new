<script setup lang="ts">
  ...
  function variantClasses(variant: Variant) {
    return {
      "bg-purple-500 hover:bg-purple-600 dark:bg-purple-600 text-white dark:hover:bg-purple-700":
        variant === "primary",
      "bg-gray-200/50 hover:bg-gray-300/70 dark:bg-purple-500/20 text-gray-700 dark:text-white dark:hover:bg-purple-500/50":
        variant === "secondary",
      "dark:text-white hover:underline dark:hover:text-gray-200 text-gray-700 hover:text-gray-500":
        variant === "link",
...
<template>
  ...
  <a v-else-if="href" :href="!disabled ? href : ''" :class="classes" :aria-disabled="disabled">
    <span v-if="srText" class="sr-only">{{ srText }}</span>
    <slot :icon-size-classes="iconSizeClasses" />
  </a>
