<template>
  <header
    class="bg-white/60 dark:bg-gray-900/60 backdrop-blur border-b border-gray-200 dark:border-gray-800 -mb-px sticky top-0 z-[99999]">
    <div class="mx-auto px-4 sm:px-6 lg:px-8 max-w-7xl flex items-center justify-between gap-3 h-16 lg:h-20">
      ...
      <slot name="center">
        <ul
          class="items-center gap-x-10 hidden lg:flex border border-gray-300/70 shadow-sm dark:border-gray-700/60 dark:border-gray-400 dark:bg-gray-600/10 px-6 py-1.5 rounded-full">
      ...
      <div class="flex items-center justify-end lg:flex-1 gap-2">
        ...
        <slot name="panel-button">
          <Button icon-only variant="secondary" class="lg:hidden" aria-label="Open Menu" @click="togglePanel">
            <Icon v-if="!isPanelOpen" name="i-heroicons-bars-3-20-solid" class="flex-shrink-0 h-5 w-5"
              aria-hidden="true" />
            <Icon v-if="isPanelOpen" name="i-heroicons-x-mark" class="flex-shrink-0 h-5 w-5" aria-hidden="true" />
          </Button>
        </slot>
      </div>
    </div>
    <div v-if="isPanelOpen"
      ...
      <div class="h-28 overflow-y-auto">
        <ul class="gap-y-5 flex flex-col rounded-full px-3">
          <li v-for="item in props.links" :key="item.to" class="relative">
            <NuxtLink class="text-sm/6 font-semibold flex items-center gap-1 hover:text-purple-600 transition-colors"
              :to="item.to">
              {{ item.label }}
            </NuxtLink>
          </li>
