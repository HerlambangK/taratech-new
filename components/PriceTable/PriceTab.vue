    ...
    <div
      ...
      <div class="mb-5">
        ...
        <div v-if="basic" class="mb-2 inline-flex items-baseline">
          ...
          <span v-show="typeof props.price.basic === 'number'" class="font-medium text-slate-500">/mo</span>
        </div>
        <div v-else class="mb-2 inline-flex items-baseline">
          ...
          <span v-show="typeof props.price.custom === 'number'" class="font-medium text-slate-500">/mo</span>
        </div>

        <div class="mb-5 text-sm text-slate-500">
          {{ props.planDescription }}
        </div>
        <a :href="`https://wa.me/6281578401214?text=Halo, saya tertarik dengan paket ${props.planName}`"
          target="_blank">
          <Button class="w-full"> Purchase Plan </Button>
        </a>
      </div>

      <div class="mb-3 font-medium text-slate-900 dark:text-slate-200">
        Includes:
      </div>
